...
class LongitudinalCompatibility(StrictModel):
    schemaVersion: Literal["longitudinal-compatibility.v0.1"]="longitudinal-compatibility.v0.1"
    subjectRef: str
    ...
    def validate_status(self):
        if self.status=="compatible" and not self.conclusionAllowed: raise ValueError("COMPATIBLE_MUST_ALLOW_CONCLUSION")
        if self.status=="blocked" and self.conclusionAllowed: raise ValueError("BLOCKED_MUST_NOT_ALLOW_CONCLUSION")
        return self
...
class QuantitativeMetric(StrictModel):
    ...
    def validate_value(self):
        if self.status in {"not_available","blocked"} and self.value is not None: raise ValueError("UNAVAILABLE_METRIC_MUST_HAVE_NULL_VALUE")
        return self
...
class UC2QuantitativeAssessment(StrictModel):
    schemaVersion: Literal["uc2-quantitative-assessment.v0.1"]="uc2-quantitative-assessment.v0.1"
    assessmentId: str
    subjectRef: str
    sessionIds: list[str]
...
class UC2AssessmentRequest(StrictModel):
    scenarioId: Literal["golden_uc2_longitudinal","uc2_protocol_incompatible","uc2_missing_baseline","uc2_qc_fail_session","uc2_bilateral_unavailable"]
