#!/usr/bin/env python3
"""Import -> QC -> preprocessing -> windowing -> RMS/MAV feature rows."""
...
from extractor import TimeDomainFeatureExtractor  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Chạy offline pipeline đến RMS/MAV theo time-domain windows."
    )
    ...
    parser.add_argument("--feature-config", type=Path, default=Path("services/feature-extraction-service/configs/features_semg_v0.1.yaml"))
...
def _blocked_preprocess_result(session_id: str, reason_codes: tuple[str, ...]) -> PreprocessingRunResult:
    return PreprocessingRunResult(
        ...
        steps=(),
        signal=None,
        limitations=("Import bị từ chối nên không có tín hiệu để trích feature.",),
    )
...
def _write_csv(payload: dict[str, Any], output_path: Path) -> None:
    ...
    fieldnames = [
        "feature_row_id", "session_id", "channel_id", "muscle", "side", "role",
        "phase_id", "profile_id", "window_id", "window_index", "start_time_s",
        "end_time_exclusive_s", "center_time_s", "sample_count", "status",
        "rms_uV", "mav_uV", "reason_codes", "feature_extractor_id",
        "windowing_config_id", "preprocess_config_id", "source_signal_hash_sha256",
    ...
    with output_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for row in payload.get("rows", []):
            ...
            writer.writerow({
                ...
                "end_time_exclusive_s": row["window"]["end_time_exclusive_s"],
                "center_time_s": row["window"]["center_time_s"],
                "sample_count": row["window"]["sample_count"],
                "status": row["status"],
                "rms_uV": features.get("rms", {}).get("value", ""),
                "mav_uV": features.get("mav", {}).get("value", ""),
                "reason_codes": "|".join(row.get("reason_codes", [])),
                "feature_extractor_id": row["provenance"]["feature_extractor_id"],
                "windowing_config_id": row["provenance"]["windowing_config_id"],
                "preprocess_config_id": row["provenance"]["preprocess_config_id"],
                "source_signal_hash_sha256": row["provenance"]["source_signal_hash_sha256"],
...
def main() -> int:
    ...
    feature_result = TimeDomainFeatureExtractor(feature_config).run(windowing_result)
    ...
    if args.print_json:
        ...
        print(f"FEATURES {payload['status'].upper()}: session={payload['session_id']} downstream_allowed={payload['downstream_allowed']}")
        print(f"rows={summary['total_row_count']} computed={summary['computed_row_count']} not_computed={summary['not_computed_row_count']} usable_ratio={summary['usable_window_ratio']:.6f}")
