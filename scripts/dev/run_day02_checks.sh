...
uv run python scripts/dev/validate_day02_workflow_time_motion.py > /tmp/day02_validate.json
...
set +e
PYTEST_OUTPUT="$(uv run pytest -q qa-validation/automated-tests/governance/test_day02_workflow_time_motion.py -p no:cacheprovider 2>&1)"
PYTEST_RC=$?
set -e
printf '%s\n' "$PYTEST_OUTPUT"

printf '%s\n' '[DAY02] 3/4 Finalize validation report'
uv run python - "$PYTEST_RC" "$PYTEST_OUTPUT" <<'PY'
import json, re, sys
from pathlib import Path
...
uv run python scripts/dev/check_day02_artifacts.py
