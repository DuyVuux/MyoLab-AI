"""Application service coordinating the domain state machine and the dual-log audit store."""
...
ALLOWED_ROLES = {"REVIEWER", "TECHNICAL_REVIEWER", "ADMIN"}
...
class ReviewWorkflowAppService:
    """Application service coordinating the domain state machine and the dual-log audit store."""
    
    def __init__(self, audit_store: Any):
        # We type hint as Any to prevent tight coupling or circular dependencies 
        # if PYTHONPATH isn't set perfectly
        self._audit_store = audit_store

    def execute_action(self, req: ReviewActionRequest) -> ReviewActionResponse:
        """
        Execute a human-review action, transitioning the state and securely 
        logging the event to the dual-stream audit store with idempotency.
        """
        _validate_request(req)
        action_id = logical_action_id(req)
        
        # 1. Idempotency Check
        # Depending on the mock/store implementation, find by logical_action_id
        if hasattr(self._audit_store, "research") and hasattr(self._audit_store.research, "find"):
            existing = self._audit_store.research.find("logical_action_id", action_id)
            if existing is not None:
                return _response_from_event(existing, True)

        # 2. Domain logic execution (pure)
        ...
        result = transition(state=req.current_state, action=transition_action, ctx=ctx)

        # 3. Formulate Dual-Log Audit Events
        ...
        # 4. Append to logs (Eventual Consistency chosen via Plan Option A)
        if hasattr(self._audit_store, "append_research"):
            self._audit_store.append_research(research_event)
        
        if hasattr(self._audit_store, "append_compliance"):
            try:
                self._audit_store.append_compliance(compliance_event)
            except Exception:
                # Embrace eventual consistency/reconciliation: do not rollback research stream.
