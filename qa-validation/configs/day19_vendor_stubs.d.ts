...
declare module "react" {
  ...
  export interface ChangeEvent<T> { target: T; currentTarget: T; }
  export interface FormEvent<T> { currentTarget: T; preventDefault(): void; }
  export function useState<T>(initial: T | (() => T)): [T, (value: T | ((previous: T) => T)) => void];
  export function useEffect(effect: () => void | (() => void), deps: readonly unknown[]): void;
  export function useReducer<S, A>(reducer: (state: S, action: A) => S, initial: S): readonly [S, (action: A) => void];
