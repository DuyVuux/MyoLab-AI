...
export function validateUncertainty(uncertainty) {
  const u=uncertainty ?? {};
  if (u.uncertainty_type === 'RULE_CONFIDENCE') {
    if (u.calibrated_probability != null) throw new Error('RULE_CONFIDENCE_MUST_NOT_BE_PROBABILITY');
    if (u.conformal_set != null) throw new Error('RULE_CONFIDENCE_MUST_NOT_BE_CONFORMAL');
  }
...
export function buildMetricEvidenceView({metric, quality_eligibility=null, distribution_support, uncertainty, claim_scope='RESEARCH_ONLY'}) {
  if (claim_scope !== 'RESEARCH_ONLY') throw new Error('RESEARCH_ONLY_REQUIRED');
  validateUncertainty(uncertainty);
  if (!metric?.metric_name || !metric?.status) throw new Error('METRIC_IDENTITY_REQUIRED');
  if (['NOT_AVAILABLE','UNSUPPORTED','BLOCKED','ABSTAINED'].includes(metric.status) && metric.value != null) {
    throw new Error('UNSUPPORTED_OR_BLOCKED_METRIC_MUST_BE_NULL');
  }
  const reasons=[...(metric.reason_codes ?? []), ...(quality_eligibility?.reason_codes ?? [])];
  ...
  return Object.freeze({
    ...
    distribution_support:Object.freeze({
      ...
      method_status:distribution_support?.method_status ?? distribution_support?.ood_method_status ?? 'NOT_IMPLEMENTED',
...
export function explainMetric(view) {
  const parts=[`${view.metric_name}: ${view.status_label}.`];
  if (view.value == null) parts.push('No numeric value is presented.');
  if (view.reason_codes.length) parts.push(`Reason: ${view.reason_codes.join(', ')}.`);
  parts.push(`Distribution support: ${view.distribution_support.status}.`);
  if (view.uncertainty?.uncertainty_type === 'RULE_CONFIDENCE') parts.push('Rule confidence is ordinal evidence, not a calibrated probability.');
  if (view.distribution_support.score == null) parts.push('No validated OOD score is available.');
  return parts.join(' ');
}
