...
export function validateWindowIdentity(w) {
  const required=['window_id','session_id','channel_id','source_id','start_sample','end_sample_exclusive','context_start_sample','context_end_sample_exclusive','sampling_rate_hz'];
  for (const key of required) if (w?.[key] === undefined || w?.[key] === null) throw new Error(`WINDOW_IDENTITY_MISSING_${key.toUpperCase()}`);
...
export function buildSeries({kind, values, fs_hz, units, source_ref, manifest_id=null, profile_id=null, mask=null, start_time_s=0}) {
  ...
  return Object.freeze({
    ...
    profile_id,
    start_time_s,
    values: Object.freeze(values.map(finiteOrNull)),
    mask: Object.freeze(mask ? [...mask] : values.map(()=>false)),
  });
}

export function toPoints(series) {
  return series.values.map((y,i)=>({x:series.start_time_s+i/series.fs_hz,y,masked:Boolean(series.mask[i])}));
}

export function decimateMinMax(points, maxPoints=2000) {
...
export function maskIntervals(series) {
  const intervals=[];
  let start=null;
  for (let i=0;i<series.mask.length;i++) {
    ...
    if ((!series.mask[i] || i===series.mask.length-1) && start!==null) {
      ...
      intervals.push({start_s:series.start_time_s+start/series.fs_hz,end_s:series.start_time_s+end/series.fs_hz});
      start=null;
    }
  }
  return intervals;
...
export function buildSignalViewerModel({window_identity, raw, processed, max_points=2000}) {
  ...
  return Object.freeze({
    ...
    window_identity:Object.freeze({...window_identity}),
    panels:Object.freeze([
      Object.freeze({kind:'RAW',units:raw.units,source_ref:raw.source_ref,manifest_id:null,profile_id:null,points:Object.freeze(decimateMinMax(toPoints(raw),max_points)),mask_intervals:Object.freeze(maskIntervals(raw))}),
      Object.freeze({kind:'PROCESSED',units:processed.units,source_ref:processed.source_ref,manifest_id:processed.manifest_id,profile_id:processed.profile_id,points:Object.freeze(decimateMinMax(toPoints(processed),max_points)),mask_intervals:Object.freeze(maskIntervals(processed))}),
    ]),
  });
}
