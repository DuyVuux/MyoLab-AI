...
export function classifyAttention(caseRecord) {
...
export function buildQueueItem(caseRecord) {
...
export function buildQueueSummary(caseRecords) {
  const items = caseRecords.map(buildQueueItem);
  items.sort((a, b) => a.priority - b.priority || a.case_id.localeCompare(b.case_id));
  const counts = Object.fromEntries(Object.keys(PRIORITY).map((k) => [k, 0]));
  for (const item of items) counts[item.attention] += 1;
  return Object.freeze({
...
export function assertNoFalseFinalState(item) {
  ...
  if (item.claim_scope !== 'RESEARCH_ONLY') throw new Error('RESEARCH_ONLY_BANNER_REQUIRED');
  return true;
}
