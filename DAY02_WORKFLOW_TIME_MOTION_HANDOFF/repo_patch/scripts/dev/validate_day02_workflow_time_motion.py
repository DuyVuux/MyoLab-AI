...
def main() -> int:
    ...
    for p in [obs_path,schema_path,workflow_path,protocol_path,trace_path,oq_path,synth_path]:
        if not p.is_file(): fail(f'Missing required artifact: {p.relative_to(ROOT)}', failures)
    if failures:
        print(json.dumps({'status':'FAIL','failures':failures},ensure_ascii=False,indent=2)); return 2

    obs=load_yaml('clinical/studies/time-motion-observation-form.v0.1.yaml')
    schema=json.loads(schema_path.read_text(encoding='utf-8'))
    try: jsonschema.Draft202012Validator(schema).validate(obs)
    except Exception as e: fail(f'Observation schema validation failed: {e}', failures)

    dp=obs['data_policy']
    for k in ['direct_phi_allowed','raw_signal_allowed','screenshots_allowed','patient_name_allowed','mrn_allowed']:
        if dp.get(k) is not False: fail(f'{k} must be false', failures)

    allowed=set(obs['evidence_policy']['allowed_statuses'])
    if allowed != EVIDENCE: fail('Evidence status enum differs from frozen DAY01 taxonomy', failures)

    tc=set(obs['coding_dictionary']['time_classes'])
    for item in ['CLINICIAN_DATA_HANDS_ON','TECHNICIAN_DATA_HANDS_ON','SYSTEM_ACTIVE','WAITING_BLOCKED','REMEASUREMENT','CLINICAL_INTERPRETATION']:
        if item not in tc: fail(f'Missing time class {item}', failures)
    ...
    forbidden_suffixes={'.joblib','.pkl','.pickle','.pt','.pth','.onnx','.h5','.hdf5','.mat','.c3d'}
    for p in ROOT.rglob('*'):
        if p.is_file() and p.suffix.lower() in forbidden_suffixes:
            fail(f'Forbidden model/raw-data-like artifact: {p.relative_to(ROOT)}', failures)
